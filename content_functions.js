bootBoxFactory = function(mc) {
    ...
    var template = "<p><b>Example</p></b><div class='clause-card'>{{markRendered}}</div><div><p><b>Instructions</b></p><p>{{parameterExplanation}}</p></div><div><p><b>Raw Text (Markdown)</b></p><textarea id='copy-text' rows=5 style='width:96%;padding:10px;font-size:14px;'>{{copyText}}</textarea></div>"
